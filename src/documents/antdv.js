...
| getPopupContainer | 菜单渲染父节点。默认body,若遇到菜单滚动定位问题,修改为滚动的区域,并相对其定位。 | function(triggerNode) | () => document.body |
| overlay(slot-scope) | 菜单 | Menu | — |
| overlayClassName | 下拉根元素的类名称 | string | — | 
| overlayStyle | 下拉根元素的样式 | object | — | 
| placement | 菜单弹出位置 | String  | bottomLeft |
| trigger | 触发下拉的行为, 移动端不支持 hover | Array | ['hover'] |
...
| openKeys(v-model) | 当前展开的 SubMenu 菜单项 key 数组 | stringArray |  |
| selectable | 是否允许选中 | boolean | true |
| selectedKeys(v-model) | 当前选中的菜单项 key 数组 | stringArray |  |
...
| disabled | 是否禁用 | boolean | — | false |
| filterOption | 是否根据输入项进行筛选。为函数时接收 inputValue option 两个参数,当 option 符合筛选条件时,应返回 true,反之则返回 false | boolean or function(inputValue, option) | — | true |
| optionLabelProp | 回填到选择框的 Option 的属性值,默认是 Option 的子元素。比如在子元素需要高亮效果时,此值可以设为 value | string | — | children |
| placeholder | 输入框提示 | string , slot | — | - |
| value(v-model) | 指定当前选中的条目 | string,string[] ,Array<{ key: string, label: string,vNodes }> | — | - |
...
| placeholder | 输入框提示文字 | string RangePicker[] | — | — |
| popupStyle | 额外的弹出日历样式 | object | — | {} |
| dropdownClassName | 额外的弹出日历样式 | string | — | — |
| size | 输入框大小,large:40px,small:24px,默认32px | string | - | - |
| suffixIcon | 自定义的选择框后缀图标 | VNode   slot | - | - |
| inputReadOnly | 设置输入框为只读（避免在移动设备上打开虚拟键盘） | boolean | - | - |
| align | 该值将合并到 placement 的配置中 | Object | — | — |
| valueFormat | 可选,绑定值的格式,对 value、defaultValue、defaultPickerValue 起作用。不指定则绑定值为 moment 对象 | string | 见日期格式 | — |
...
| 事件名称 | 说明 | 回调参数 |
| :--- | :--- | :--- |
| openChange | 弹出日历和关闭日历的回调 | (status) |
| panelChange | 日期面板变化时的回调 | (value, mode) |
| change | 日期范围发生变化的回调 | (dates: [moment, moment]  [string, string], dateStrings: [string, string]) |
...
| 参数 | 说明 | 类型 | 可选值 | 默认值 |
| :--- | :--- | :--- | :--- | :--- |
| form | this.$form.createForm(this, options) | object | — | - |
...
| colon | 是否显示label后面的冒号 (只有在属性 layout 为 horizontal 时有效) | boolean | — | true |\n
...
| 方法 | 说明 | 类型 |
| :--- | :--- | :--- |
| validate | 对整个表单进行校验的方法,参数为回调函数。会在校验结束后被调用,并传入两个参数：是否校验成功和未通过校验的字段。若不传入回调函数,则会返回一个 promise | (callback: Function(boolean, object)) |
| validateField | 对部分表单字段进行校验的方法 | (props: array | string, callback: Function(errorMessage: string)) |
| resetFields | 对整个表单进行重置,将所有字段值重置为初始值并移除校验结果 | - |
| clearValidate | 移除表单项的校验结果。传入待移除的表单项的 prop 属性或者数组,不传则移除整个表单的校验结果 | (props: array | string) |
...
| dropdownMenuStyle | dropdown 菜单自定义样式 | Object | — | - |
| filterOption | 是否根据输入项进行筛选。当其为一个函数时,会接收 inputValue option 两个参数,符合筛选条件时返回 true,反之false | boolean or function(inputValue, option) | — | - |
| firstActiveValue | 默认高亮的选项 | string,string[] | — | - |
| getPopupContainer | 菜单渲染父节点。默认渲染到 body 上,遇到菜单滚动定位问题,修改为滚动的区域并相对其定位 | Function(triggerNode) | — | () => document.body |
...
| optionLabelProp | 回填到选择框的 Option 的属性值。子元素需要高亮效果时,此值可以设为 value | string | — | - |
...
| step | 步长,必须大于 0,可被 (max - min) 整除。| number/null | 1 | 
...
| disabled | 是否禁用 | boolean | — | false |
| filterOption | 接收 inputValue option 两个参数,符合筛选条件时,返回 true,反之false。 | (inputValue, option): boolean | — | false |
...
| filterTreeNode | 是否根据输入项进行筛选,默认用 treeNodeFilterProp 的值作为要筛选的属性值 | boolean/Function(inputValue: string, treeNode: TreeNode) (函数需要返回 bool 值) | — | Function |  
| getPopupContainer | 菜单渲染父节点。默认渲染到 body 上,遇到菜单滚动定位问题,修改为滚动的区域,并相对其定位 | Function(triggerNode) | — | () => document.body |  
...
| 参数 | 说明 | 类型 | 默认值 | 版本 |
| :--- | :--- | :--- | :--- | :--- |
| activeKey(v-model) | 当前激活 tab 面板的 key | stringArray,string | 默认无,accordion 模式下默认第一个元素 |  |
...
| pagination | 分页器,参考pagination,设为 false 时不展示和进行分页 | object |  |
...
| transformCellText | 数据渲染前可以再次改变,一般用于空数据的默认配置,可以通过 ConfigProvider全局统一配置 | Function({ text, column, record, index }) => any,此处的 text 是经过其它定义单元格 api 处理后的数据,有可能是 VNode , string , number 类型 | - |
...
- 'expandFixed'
    ...
    | selectedRowKeys | 指定选中项的 key 数组,需要和 onChange 进行配合 | stringArray| Array |
    | selections | 自定义选择项 [配置项](#selection), 设为 'true' 时使用默认选择项 | objectArray , boolean | true |
...
| dataIndex | 列数据在数据项中对应的路径,支持通过数组查询嵌套路径 | string , stringArray | - |
| defaultFilteredValue | 默认筛选值 | stringArray | - |
| defaultSortOrder | 默认排序顺序 | 'ascend' , 'descend' | - |
| filterDropdown | 可以自定义筛选菜单,此函数只负责渲染图层,需要自行编写各种交互 | VNode | - |
| filterDropdownVisible | 用于控制自定义筛选菜单是否可见 | boolean | - |
| filtered | 标识数据是否经过过滤,筛选图标会高亮 | boolean | false |
| filteredValue | 筛选的受控属性,外界可用此控制列的筛选状态,值为已筛选的 value 数组 | stringArray | - |
| filterIcon | 自定义 filter 图标。 | VNode , ({filtered: boolean, column: Column}) => vNode | false |
| filterMultiple | 是否多选 | boolean | true |
| filters | 表头的筛选菜单项 | objectArray | - |
...
| checkedKeys(v-model) | （受控）选中复选框的树节点（注意：父子节点有关联,如果传入父节点 key,则子节点自动选中；相应当子节点 key 都传入,父节点也自动选中。当设置'checkable'和'checkStrictly',它是一个有'checked'和'halfChecked'属性的对象,并且父子节点的选中与否不再关联 | stringArray , number\[] , {checked: stringArray , number\[], halfChecked: stringArray , number\[]} | \[] |
...
| expandedKeys(v-model) | （受控）展开指定的树节点 | stringArray , number\[] | \[] |
| filterTreeNode | 按需筛选树节点（高亮）,返回 true | function(node) | - |
| loadData | 异步加载数据 | function(node) | - |
| loadedKeys | （受控）已经加载的节点,需要配合 'loadData' 使用 | stringArray , number\[] | \[] |
| multiple | 支持点选多个节点（节点本身） | boolean | false |
| selectable | 是否可选中 | boolean | true |  |
| selectedKeys(v-model) | （受控）设置选中的树节点 | stringArray , number\[] | - |
...
| targetOffset | 锚点滚动偏移量,默认与 offsetTop 相同,| number | 'offsetTop' | 
