...
const vue_tags = require("./vue-tags");
const vue_attributes = require("./vue-attributes");
const documents = require("./documents");
const documents_attr = require("./documents-attr");
const fs = require('fs');
...
function encodeDocsUri (query) {
  return vscode.Uri.parse(`${exports.SCHEME}://search?${JSON.stringify(query)}`);
}
...
class App {
      ...
      if (tag === vf.name) {
        ...
        if (line < countLine) {
          ...
          while (!/\s*<\/script>\s*/gi.test(editor.document.lineAt(line).text.trim())) {
            if (/\s*components\s*:\s*{.*}.*/gi.test(editor.document.lineAt(line).text.trim())) {
              ...
              editor.edit((editBuilder) => {
                ...
                editBuilder.insert(new vscode.Position(importLine, 0), importString);
                editBuilder.insert(new vscode.Position(line, insertPos), ', ' + name);
              });
              break;
            }
            if (hasComponents && /\s*},?\s*$/gi.test(editor.document.lineAt(line).text.trim())) {
              ...
              editor.edit((editBuilder) => {
                importString = importString.replace(/\\/gi, '/');
                editBuilder.insert(new vscode.Position(importLine, 0), importString);
                editBuilder.insert(new vscode.Position(line - 1, editor.document.lineAt(line - 1).text.length), ',\n' + empty + name);
              });
              break;
            }
          ...
          if (prorityInsertLine > 0) {
            editor.edit((editBuilder) => {
              importString = importString.replace(/\\/gi, '/');
              editBuilder.insert(new vscode.Position(importLine - 1, 0), importString);
              editBuilder.insert(new vscode.Position(prorityInsertLine - 1, editor.document.lineAt(prorityInsertLine - 1).text.length), `\n\t${baseEmpty}components: { ${name} },`);
            });
            break;
          }
          if (secondInsertLine > 0) {
            let veturConfig = vscode.workspace.getConfiguration('vetur');
            ...
            editor.edit((editBuilder) => {
              importString = importString.replace(/\\/gi, '/');
              editBuilder.insert(new vscode.Position(importLine, 0), importString);
              editBuilder.insert(new vscode.Position(secondInsertLine, editor.document.lineAt(secondInsertLine).text.length), `\n${spaceAdd}components: { ${name} },`);
            });
          }
        }
        break;
  ...
  static traverse (poster, search) {
    ...
    let rootPathes = fs.readdirSync(vscode.workspace.rootPath);
    ...
    for (let i = 0; i < rootPathes.length; i++) {
      ...
      if (cond(rootPath)) {
        let stat = fs.statSync(path.join(vscode.workspace.rootPath, rootPath));
        if (stat.isDirectory()) {
  ...
  static traverseHandle (postPath, vueFiles, prefix, poster, search) {
    let fileDirs = fs.readdirSync(path.join(vscode.workspace.rootPath, postPath));
    for (let i = 0; i < fileDirs.length; i++) {
      const rootPath = fileDirs[i];
      if (!(rootPath.charAt(0) === '.')) {
        let dir = path.join(postPath, rootPath);
        let stat = fs.statSync(path.join(vscode.workspace.rootPath, dir));
        if (stat.isDirectory()) {
  ...
  autoComplement () {
      ...
      if (search) {
        ...
        vueFiles.forEach(vf => {
          suggestions.push({
            ...
            insertText: new vscode.SnippetString(`${vf.name}$0></${vf.name}>`),
    ...
    editor.edit((editBuilder) => {
      editBuilder.insert(new vscode.Position(editor.selection.anchor.line, txt.length + 1), replaceTxt);
    });
  ...
  deleteComplete () {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let index = 0; index < selections.length; index++) {
          ...
          if (selection.start.line === selection.end.line && selection.start.character === selection.end.character) {
            if (selection.anchor.character > 0) {
              selectionList.push(new vscode.Selection(new vscode.Position(selection.anchor.line, selection.anchor.character - 1), selection.anchor));
            }
            else if (selection.anchor.line > 0) {
              let len = editor.document.lineAt(selection.anchor.line - 1).text.length;
              selectionList.push(new vscode.Selection(new vscode.Position(selection.anchor.line - 1, len), selection.anchor));
            }
          }
          else {
      ...
      if (vscode.window.activeTextEditor.selection.start.line === vscode.window.activeTextEditor.selection.end.line
        && vscode.window.activeTextEditor.selection.start.character === vscode.window.activeTextEditor.selection.end.character) {
        // 首行
        if (editor.selection.anchor.line === 0) {
          if (editor.selection.anchor.character > 0) {
            yield editor.edit((editBuilder) => {
              editBuilder.delete(new vscode.Selection(new vscode.Position(editor.selection.anchor.line, editor.selection.anchor.character - 1), editor.selection.anchor));
            });
          }
        }
        else {
          ...
          if (isLineEmpty) {
            ...
            yield editor.edit((editBuilder) => {
              editBuilder.delete(new vscode.Selection(new vscode.Position(line, preText.length), editor.selection.anchor));
            });
          }
          else {
            let startPosition;
            let endPosition = editor.selection.anchor;
            let preLineText = editor.document.getText(new vscode.Range(new vscode.Position(endPosition.line, 0), endPosition));
            if (endPosition.character === 0 || preLineText.trim() === '') {
              startPosition = new vscode.Position(endPosition.line - 1, editor.document.lineAt(endPosition.line - 1).text.length);
            }
            else {
              startPosition = new vscode.Position(endPosition.line, endPosition.character - 1);
              // 对{}, (), [], '', "", <>进行成对删除处理
              let txt = editor.document.getText(new vscode.Range(new vscode.Position(endPosition.line, endPosition.character - 1), endPosition));
              if (editor.document.lineAt(endPosition.line).text.length > endPosition.character) {
                let nextTxt = editor.document.getText(new vscode.Range(endPosition, new vscode.Position(endPosition.line, endPosition.character + 1)));
                if ((txt === '{' && nextTxt === '}')
                  || (txt === '(' && nextTxt === ')')
                  || (txt === '\'' && nextTxt === '\'')
                  || (txt === '"' && nextTxt === '"')
            ...
            yield editor.edit((editBuilder) => {
              editBuilder.delete(new vscode.Selection(startPosition, endPosition));
            });
          }
        }
      }
  ...
  selectHtmlBlock (editor, lineText, startPosition) {
    ...
    let beginPosition = new vscode.Position(startPosition.line, startPosition.character + lineText.length - lineText.replace(/\s*(.*)/, '$1').length);
    ...
    while (lineText) {
      ...
      if (textTagPos === 0) {
        ...
        if (Array.isArray(tagStack) && tagStack.length === 0) {
          editor.selection = new vscode.Selection(beginPosition, new vscode.Position(lineCurrent, col));
          break;
        }
        ...
        if (startTagMatch) {
          ...
          if (isNoIncludeTag) {
            ...
            editor.selection = new vscode.Selection(beginPosition, new vscode.Position(lineCurrent, indexLast + 2));
            break;
          }
          if (Array.isArray(tagStack)) {
        ...
        if (lineText.indexOf('/>') !== -1 && Array.isArray(tagStack) && tagStack.length === 1) {
          ...
          editor.selection = new vscode.Selection(beginPosition, new vscode.Position(lineCurrent, col + tagCloseIndex + 2));
          break;
        }
  ...
  selectJsBlock (editor, lineText, startPosition, type) {
    ...
    while (lineCurrent <= lineCount) {
      ...
      if (braceLeftCount === 0) {
        ...
        editor.selection = new vscode.Selection(startPosition, new vscode.Position(lineCurrent, pos + extra));
        return;
      }
      ++lineCurrent;
  ...
  selectLineBlock (editor, lineText, startPosition) {
    ...
    editor.selection = new vscode.Selection(new vscode.Position(startPosition.line, beginPos), new vscode.Position(startPosition.line, endPos));
  }
  // 块选择
  blockSelect () {
    let editor = vscode.window.activeTextEditor;
    ...
    if (lineText.length > 0 && startPosition.character === 0 && lineText[startPosition.character] === '[') {
      ...
      this.selectJsBlock(editor, lineText.substring(startPosition.character - 1, lineText.length), new vscode.Position(startPosition.line, startPosition.character - 1), 'array');
    }
      ...
      this.selectJsBlock(editor, lineText.substring(startPosition.character - 1, lineText.length), new vscode.Position(startPosition.line, startPosition.character - 1), 'json');
    }
    else if (lineText.length > 0 && startPosition.character < lineText.length && lineText[startPosition.character] === '{') {
      this.selectJsBlock(editor, lineText.substring(startPosition.character, lineText.length), startPosition, 'json');
    }
      ...
      this.selectJsBlock(editor, lineText, new vscode.Position(startPosition.line, lineText.length - lineText.replace(/\s*/, '').length), 'array');
    }
      ...
      this.selectJsBlock(editor, lineText, new vscode.Position(startPosition.line, lineText.length - lineText.replace(/\s*/, '').length), 'function');
    }
    else {
      // 在本行选择
...
class ElementCompletionItemProvider {
  ...
  getPreAttr () {
    ...
    let parsedTxt = this._document.getText(new vscode.Range(this._position.line, start, this._position.line, end));
  ...
  getTagSuggestion () {
    ...
    for (let tag in vue_tags.default) {
      suggestions.push(this.buildTagSuggestion(tag, vue_tags.default[tag], id));
      id++;
    }
    return suggestions;
  }
  ...
  getAttrSuggestion (tag) {
    ...
    for (let attr in vue_attributes.default) {
      const attrItem = this.getAttrItem(tag, attr);
  ...
  buildTagSuggestion (tag, tagVal, id) {
    ...
    function build (tag, { subtags, defaults }, snippets) {
      ...
      // 子标签
      if (subtags) {
        subtags.forEach(item => build(item, vue_tags.default[item], snippets));
        snippets.push(`</${tag}>`);
      }
      else {
        // 关闭标签
    ...
    return {
      label: tag,
      sortText: `0${id}${tag}`,
      insertText: new vscode.SnippetString(prettyHTML('<' + snippets.join(''), { indent_size: this.size }).substr(1)),
  ...
  buildAttrSuggestion ({ attr, tag, bind, method }, { description, type, global, framework }) {
    if ((method && type === "method") || (bind && type !== "method") || (!method && !bind)) {
      ...
      if (vue_tags.default[tag] && vue_tags.default[tag].framework) {
        detail += vue_tags.default[tag].framework;
      }
      if (global) {
      ...
      return {
        label: attr,
        insertText: (type && (type === 'flag')) ? `${attr} ` : new vscode.SnippetString(`${attr}=${this.quotes}$1${this.quotes}$0`),
        kind: (type && (type === 'method')) ? vscode.CompletionItemKind.Method : vscode.CompletionItemKind.Property,
        detail,
  ...
  getAttrValues (tag, attr) {
    ...
    if (!options && attrItem) {
      if (attrItem.type === 'boolean') {
        ...
        options = vue_attributes.default['icons'];
      }
      else if (attrItem.type === 'shortcut-icon') {
        options = [];
        vue_attributes.default['icons'].forEach(icon => {
  ...
  // 获取标签包含的属性
  getTagAttrs (tag) {
    return (vue_tags.default[tag] && vue_tags.default[tag].attributes) || [];
  }
  // 获取属性项
  getAttrItem (tag, attr) {
    return vue_attributes.default[`${tag}/${attr}`] || vue_attributes.default[attr];
  }
  // 属性值开始
  ...
  // 是否是关闭标签
  isCloseTag () {
    let txt = this._document.getText(new vscode.Range(new vscode.Position(this._position.line, 0), this._position)).trim();
  ...
  getCloseTagSuggestion () {
    ...
    if (exclude.indexOf(tag) === -1) {
      ...
      let newPosition = vscode.window.activeTextEditor.selection.active.translate(0, 0);
      vscode.window.activeTextEditor.selection = new vscode.Selection(newPosition, newPosition);
    }
  }
  // 判断是否是{}括号开始
  isBrace () {
    let startPosition = new vscode.Position(this._position.line, this._position.character - 2);
    return /[^{]{/gi.test(this._document.getText(new vscode.Range(startPosition, this._position)));
  }
    ...
    if (search) {
      ...
      vueFiles.forEach(vf => {
        ...
        suggestions.push({
          label: vf.name,
          sortText: `0${vf.name}`,
          insertText: new vscode.SnippetString(`${camelName} from '${filePath}'`),
  ...
  getPropAttr (documentText, tagName) {
    ...
    if (pathRegArr && pathRegArr.length > 0) {
      ...
      if (tagPath.indexOf('./') > 0 || tagPath.indexOf('../') > 0) {
        ...
        tagPath = path.join(vscode.workspace.rootPath, tagPath);
      }
        ...
        if (propMatch.length > 0) {
          propMatch.forEach((propItem, propIndex) => {
            ...
            props.push({
              ...
              insertText: new vscode.SnippetString(`:${propItem}="$0"`),
    ...
    if (emitReg && emitReg.length > 0) {
      for (let i = 0; i < emitReg.length; i++) {
        ...
        props.push({
          ...
          insertText: new vscode.SnippetString(`@${emitName}="$0"`),
...
class DocumentHoverProvider {
  ...
  provideHover (document, position) {
    ...
    textMeta = line.text.substr(posIndex, 1);
    // tag标签便利
    if (documents.default[selectText]) {
      return new vscode.Hover(documents.default[selectText]);
    }
    // 在不是标签元素情况下才获取标签名称
    if (textMeta !== "<") {
      let tagName = DocumentHoverProvider.getTag(document, position);
      if (tagName && documents_attr.default[tagName + '/' + selectText]) {
        return new vscode.Hover(documents_attr.default[tagName + '/' + selectText]);
      }
    }
    return null;
  }
