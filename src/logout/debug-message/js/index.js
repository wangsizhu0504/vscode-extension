...
class JSDebugMessage extends __1.DebugMessage {
  ...
  msg(
    ...
    if (!insertEnclosingFunction && !insertEnclosingClass && logMessagePrefix.length === 0) {
      logMessagePrefix = `${delemiterInsideMessage} `;
    }
    const debuggingMsg = `console.log(${quote}${logMessagePrefix}${
      logMessagePrefix.length !== 0 && logMessagePrefix !== `${delemiterInsideMessage} `
        ? ` ${delemiterInsideMessage} `
        : ''
    }${
      insertEnclosingClass
  ...
  objectFunctionCallLine(document, selectionLine, selectedVar) {
    ...
    const { openedElementOccurrences, closedElementOccurrences } = this.locOpenedClosedElementOccurrences(
      currentLineText,
      entities_1.LocElement.Parenthesis
    );
  ...
  spacesBefore(document, line, tabSize) {
    ...
    if (
      (!this.lineCodeProcessing.isFunctionAssignedToVariable(currentLine.text) &&
        ...
        case 'class':
          if (this.lineCodeProcessing.doesContainClassDeclaration(currentLineText)) {
            if (
              ...
              lineOfSelectedVar < this.closingElementLine(document, currentLineNum, entities_1.LocElement.Braces)
            ) {
  ...
  detectAll(document, tabSize, delemiterInsideMessage, quote) {
    ...
    for (let i = 0; i < documentNbrOfLines; i++) {
      ...
      if (turboConsoleLogMessage.test(document.lineAt(i).text)) {
        ...
        if (
          new RegExp(`${delemiterInsideMessage}[a-zA-Z0-9]+${quote},(//)?[a-zA-Z0-9]+`).test(msg.replace(/\s/g, ''))
        ) {
