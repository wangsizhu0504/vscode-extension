...
const global = require('../utils/CONST')
    ...
    try {
      ...
      setTimeout(() => {
        try {
          ...
          handleError.showErrorMessage('wszfileheader: headerAnnotation save', err)
        }
      }, 200)
    } catch (err) {
      handleError.showErrorMessage('wszfileheader: headerAnnotation', err)
    }
  })
}
    ...
    editor.edit((editBuilder) => {
      ...
      setTimeout(() => {
        try {
          ...
          handleError.showErrorMessage('wszfileheader: functionAnnotation moveCursorDesFn', err)
        }
      }, 100)
    })
  } catch (err) {
    handleError.showErrorMessage('wszfileheader: functionAnnotation', err)
  }
}
